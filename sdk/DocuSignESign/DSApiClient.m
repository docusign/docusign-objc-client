...
+(NSNumber*) nextRequestId {
    @synchronized(self) {
        ...
        DSDebugLog(@"got id %ld", nextId);
        return [NSNumber numberWithLong:nextId];
    }
}

+(NSNumber*) queueRequest {
    NSNumber* requestId = [DSApiClient nextRequestId];
    ...
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                DSDebugLog(@"reachability changed to AFNetworkReachabilityStatusUnknown");
                [DSApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusNotReachable:
                DSDebugLog(@"reachability changed to AFNetworkReachabilityStatusNotReachable");
                [DSApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusReachableViaWWAN:
                DSDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWWAN");
                [DSApiClient setOfflineState:false];
                break;

            case AFNetworkReachabilityStatusReachableViaWiFi:
                DSDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWiFi");
                [DSApiClient setOfflineState:false];
                break;
            default:
                break;
                                                               ...
                                                               success:^(AFHTTPRequestOperation *operation, id response) {
                                                                   ...
                                                                   if([self executeRequestWithId:requestId]) {
                                                                       ...
                                                                       if(operation.responseObject) {
                                                                           ...
                                                                           userInfo[DSResponseObjectErrorKey] = operation.responseObject;
                                                                       }
...
-(NSNumber*)  requestWithCompletionBlock: (NSString*) path
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [resourcePath replaceCharactersInRange:[resourcePath rangeOfString:[NSString stringWithFormat:@"%@%@%@", @"{", key, @"}"]]
                                    withString:[DSApiClient escape:obj]];
    }];
        ...
        for(NSString * key in [queryParams keyEnumerator]){
            ...
            if([queryParam isKindOfClass:[NSString class]]){
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [DSApiClient escape:key], [DSApiClient escape:[queryParams valueForKey:key]]]];
            }
                ...
                if([format isEqualToString:@"csv"]) {
                    [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                        [DSApiClient escape:key], [NSString stringWithFormat:@"%@", [values componentsJoinedByString:@","]]]];
                    ...
                    for(id obj in values) {
                        [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                            [DSApiClient escape:key], [NSString stringWithFormat:@"%@", obj]]];
                        counter += 1;
                    }
                ...
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [DSApiClient escape:key], [NSString stringWithFormat:@"%@", [queryParams valueForKey:key]]]];
            }
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[DSQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[DSObject class]]) {
